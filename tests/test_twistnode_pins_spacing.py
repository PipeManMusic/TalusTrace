...
def test_twistnode_pins_are_orthogonal_and_spaced():
    ...
    # For source node pins are on LEFT side: x should be equal, y separation should be 2 * GRID_SIZE
    assert round(h.x(), 6) == round(l.x(), 6)
    assert round(abs(h.y() - l.y()), 6) == round(GRID_SIZE * 2, 6)
    ...
    assert round(abs(h2.y() - l2.y()), 6) == round(GRID_SIZE * 2, 6)
