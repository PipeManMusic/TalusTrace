...
def test_space_rotates_selected_bundle_nodes(qapp):
    ...
    # Check that at least one of the connected node pin tips moves after rotation
    h0 = bundle.source_node.get_pin_tip_scene_pos('H')
    prev_rot = bundle.source_node._rotation
    evt = QKeyEvent(QEvent.KeyPress, Qt.Key_Space, Qt.NoModifier)
    from PySide6.QtWidgets import QApplication as _QApp
    _QApp.sendEvent(win.view, evt)
    h1 = bundle.source_node.get_pin_tip_scene_pos('H')
    # Rotation should either change rotation state or move the pin position
    assert bundle.source_node._rotation != prev_rot or not (round(h0.x(), 6) == round(h1.x(), 6) and round(h0.y(), 6) == round(h1.y(), 6))
