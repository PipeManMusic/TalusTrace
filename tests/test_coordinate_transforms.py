...
def test_mm_to_pixel_transformation(tmp_path):
    ...
    mock_tokens = {
        "dimensions": {
            "physical_scale": 254.0,  # 10x scale
            "grid_size_mm": 2.0
        }
    }
...
def test_zero_scale_safety():
    ...
    # Should fallback to default 96.0 PPI rather than crashing
    transformer = CoordinateTransformer(theme_path=Path("non_existent.json"))
    
    # FIX: Updated expectation to match the class default of 96.0 PPI
    # (10mm / 25.4 mm/in) * 96.0 px/in
    expected = (10.0 / 25.4) * 96.0
