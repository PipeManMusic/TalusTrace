...
class TwistedPair(BaseModel):
    ...
    # End A Configuration
    node_a_pos: Tuple[float, float]
    rotation_a: int = 0  # 0, 90, 180, 270

    # End B Configuration
    node_b_pos: Tuple[float, float]
    rotation_b: int = 0
...
4.2. Frontend Implementation (frontend/twisted_pair.py)We need a custom QGraphicsItem that manages the sub-items.Pythonclass TwistedPairItem(QGraphicsObject):
    def __init__(self, model):
        # 1. Create Anchors
        self.anchor_a = TwistAnchorItem(self, model.node_a_pos)
        self.anchor_b = TwistAnchorItem(self, model.node_b_pos)
        
        # 2. Create Pins (Children of Anchors or managed by Item?)
        # Managed by Item ensures they snap to Global Grid, not Local.
        self.pins_a = [PinItem(...), PinItem(...)] 
        self.pins_b = [PinItem(...), PinItem(...)]
